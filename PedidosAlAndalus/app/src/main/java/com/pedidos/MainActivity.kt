 ...
 * 4. Cuando termina, dice "enviar" y la app abre WhatsApp directamente en la conversación
 *    configurada, con el pedido ya escrito — solo falta un toque en el botón de enviar.
 *
 * Además, desde esta pantalla se puede entrar en "Programación" para dejar mensajes
 * (pedidos fijos) programados para que se envíen solos un día y hora concretos.
 */
class MainActivity : AppCompatActivity() {
    ...
    private fun iniciarPedido() {
        val telefono = Prefs.getTelefono(this)
        if (telefono.isBlank()) {
            Toast.makeText(this, "Primero configura el número de WhatsApp en Ajustes", Toast.LENGTH_LONG).show()
            startActivity(Intent(this, SettingsActivity::class.java))
            return
        }
    ...
    private fun finalizarYEnviar() {
        ...
        actualizarEstadoTexto("Abriendo WhatsApp...\n\n$mensaje")

        val telefono = Prefs.getTelefono(this)
        WhatsappUtil.abrirConMensaje(this, telefono, mensaje)
    }
